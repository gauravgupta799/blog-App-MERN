...
server.post("/user-written-blogs", verifyToken, (req, res)=>{
    const user_id = req.user;
...
server.post("/user-written-blogs-count", verifyToken, (req, res)=>{
    const user_id = req.user;
...
server.post("/delete-blog", verifyToken, (req, res)=>{
    const user_id = req.user;
    ...
    .then((blog)=>{
        ...
        .then(data=> console.log("Notifications ared deleted"))

        Comment.deleteMany({blog_id: blog._id}).then(data=> console.log("Comments are deleted"))

        User.findOneAndUpdate({ _id: user_id }, { $pull: { blog:blog._id }, $inc: { "account_info.total_posts": -1 } }).then(user => console.log("Blog is deleted"))
